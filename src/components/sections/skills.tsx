...
const SkillsSection = () => {
  ...
  return (
    <section id="skills" className="bg-portfolio-gray-50 dark:bg-portfolio-gray-800 font-sans">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {skillCategories.map((category, index) => (
            <div
              key={category.name}
              className={`card relative transition-transform hover-scale-soft ${
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {category.items.map(skill => (
                  <span
                    key={skill}
                    className="px-3 py-1 bg-portfolio-gray-100 dark:bg-portfolio-gray-700 rounded-full text-sm hover:bg-portfolio-accent/20 transition-all duration-300 font-medium shadow-sm flex items-center gap-1 font-sans border border-gray-200 dark:border-gray-700"
                  >
                    <span className="inline-block w-1.5 h-1.5 rounded-full bg-portfolio-accent"></span>
                    {skill}
                  </span>
                ))}
              </div>
